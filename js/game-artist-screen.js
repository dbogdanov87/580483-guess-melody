import {getRandom, renderScreen, changeScreen} from './util.js';
import onGameBackWelcomeScreenButton from './back-welcome-screen.js';
import resultSuccessScreen from './result-success-screen.js';
import failTriesScreen from './fail-tries-screen.js';
import failTimeScreen from './fail-time-screen.js';


const screens = [resultSuccessScreen, failTriesScreen, failTimeScreen];

const gameArtistScreen = renderScreen(`
<div id="game-artist">
...
const backButton = gameArtistScreen.querySelector(`.game__back`);
backButton.addEventListener(`click`, onGameBackWelcomeScreenButton);

const gameArtistForm = gameArtistScreen.querySelector(`.game__artist`);

gameArtistForm.addEventListener(`click`, (evt) => {
  if (evt.target.className === `artist__picture`) {
    changeScreen(getRandom(screens));
  }
});
