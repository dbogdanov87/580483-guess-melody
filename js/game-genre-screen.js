...
import gameArtistScreen from './game-artist-screen.js';
import onGameBackWelcomeScreenButton from './back-welcome-screen.js';

const gameGenreScreen = renderScreen(`
<div id="game-genre">
...
const backButton = gameGenreScreen.querySelector(`.game__back`);
backButton.addEventListener(`click`, onGameBackWelcomeScreenButton);

const replySubmitButton = gameGenreScreen.querySelector(`.game__submit`);
replySubmitButton.disabled = true;

const getCheckedAnswers = () => {
  return gameGenreScreen.querySelectorAll(`.game__input:checked`);
};

const gameTrackForm = gameGenreScreen.querySelector(`.game__tracks`);
